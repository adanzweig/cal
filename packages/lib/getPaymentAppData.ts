...
import type { appDataSchema, paymentOptionEnum } from "@calcom/app-store/mercadopagopayment/zod";
...
export default function getPaymentAppData(
  ...
  // console.log("llega hasta aqui2----------");
  const metadataApps = eventType?.metadata?.apps as unknown as EventTypeAppsList;
  if (!metadataApps) {
    return { enabled: false, price: 0, currency: "ars", appId: null };
  }
